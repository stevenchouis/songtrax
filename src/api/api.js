 ...
 */
export async function createSample(name, type, recording_data) {
    const url = `${BASEURL}sample/?api_key=${APIKEY}`;
 ...
 */
export const updateSample = async (id, name, type, recording_data) => {
    const url = `${BASEURL}sample/${id}/?api_key=${APIKEY}`;
    const currentDate = new Date(Date.now());
    const data = {
        'type': type, 
 ...
 */
export async function createSampleToLocation(sampleID, locationID) {
    const url = `${BASEURL}sampletolocation/?api_key=${APIKEY}`;
